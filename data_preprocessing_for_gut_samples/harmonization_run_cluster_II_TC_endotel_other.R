...
seurat_objects <- lapply(seurat_objects, function(x){
  ...
  if ("TC"%in%x$locations){
    
    # filter endotel_other cells
    
    x <- x[,colnames(x)%in%names(cell_names_transversum)]
    cell_names_transversum_step <- cell_names_transversum[names(cell_names_transversum)%in%names(x$orig.ident)]
    cell_names_transversum_step <- cell_names_transversum_step[match(names(x$orig.ident),
                                                         names(cell_names_transversum_step))]
    x$cell_names <- cell_names_transversum_step
    x <- subset(x, subset = cell_names == "endotel_other")
    
    #
    
    x <- subset(x = x, subset = locations == "TC")
...
saveRDS(seurat_objects, paste0("seurat_part_TC_endotel_other_",arguments,".RDS"))
