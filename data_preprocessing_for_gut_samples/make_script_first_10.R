...
sequencing_metadata <- read.csv(paste0(data_folder,"metadata_sequencing/10_first_samples.csv"))
...
sequencing_metadata$HTO.run.name <- sapply(sequencing_metadata$HTO.run.name, function(x){
  ...
  sub("_S.*","",sub("_Read.*","",x), fixed = F)
  
})

sequencing_metadata$HTO.run.name[1] <- "NGS19-J120_Souad-hashtags"
sequencing_metadata$HTO.run.name[3] <- "NGS19-J231_Yumie_B3_HTO"
...
for (i in 1:nrow(sequencing_metadata)){
  ...
  cat(paste0("cellranger count --id=",sequencing_metadata$sample[i],
             ...
             data_folder_cluster,"metadata_sequencing/TotalSeqA_feature_ref_swap.csv"))
  cat("\n")
  cat("\n")
...
file.create(paste0(data_folder, "scripts/ruler_sc_10.sh"))

sink(paste0(data_folder, "scripts/ruler_sc_10.sh"))
...
cat("\n")
cat("\n")
cat(paste0("job_1=$(sbatch sample_B1_script.sh | awk '{print $4}')"))
cat("\n")
