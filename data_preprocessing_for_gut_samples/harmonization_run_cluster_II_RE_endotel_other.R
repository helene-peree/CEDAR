...
seurat_objects <- lapply(seurat_objects, function(x){
  ...
  if ("RE"%in%x$locations){
    
    # filter endotel_other cells
    
    x <- x[,colnames(x)%in%names(cell_names_rectum)]
    cell_names_rectum_step <- cell_names_rectum[names(cell_names_rectum)%in%names(x$orig.ident)]
    cell_names_rectum_step <- cell_names_rectum_step[match(names(x$orig.ident),
                                                         names(cell_names_rectum_step))]
    x$cell_names <- cell_names_rectum_step
    
    if (!"endotel_other"%in%x$cell_names){x <- 0} else {
      
      x <- subset(x, subset = cell_names == "endotel_other")
      
      #
      
      x <- subset(x = x, subset = locations == "RE")
...
saveRDS(seurat_objects, paste0("seurat_part_RE_endotel_other_",arguments,".RDS"))
