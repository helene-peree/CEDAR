...
params.genotype_file = "$PWD/raw_data/CEDAR-clean-snps-4.vcf.gz"
params.metadata = "$PWD/raw_data/Covariates_202304.csv"
...
process metadata_residualize {
    ...
    publishDir "$params.outdir/residualised_expression_${sample_ID}_${No_peer_factors}", mode: 'copy'
...
process cis_eQTLs_conditional_for_correct {
    
    label 'medium'
    
    input:
    tuple val(sample_ID), path(conditional_results), path(genotype_file), path(genotype_index)
    ...
    Rscript ${script_snps_extraction} cis.conditional_${sample_ID} sniplist_${sample_ID}
...
process cis_residualise {
    ...
    tuple val(sample_ID), path(residualised_expression), path(conditional_vcf), path(conditional_results) 
    ...
    Rscript ${script_cis_resid} cis.conditional_${sample_ID} expression_residualised_${sample_ID} to_correct_expr_${sample_ID}.vcf.gz expression_cis_residualised_${sample_ID}
...
process trans_eQTLs_nominal_agg_hits {
    ...
    """
    zcat trans.nominal.chunk_${sample_ID}*.hits.txt.gz > trans.nominal.chunk_${sample_ID}_hits.txt
    gzip -c trans.nominal.chunk_${sample_ID}_hits.txt > trans.nominal_${sample_ID}_hits.txt.gz
...
process trans_eQTLs_nominal_agg_bins {
    ...
    """
    zcat trans.nominal.chunk_${sample_ID}*.bins.txt.gz > trans.nominal.chunk_${sample_ID}_bins.txt
    gzip -c trans.nominal.chunk_${sample_ID}_bins.txt > trans.nominal_${sample_ID}_bins.txt.gz
...
process trans_eQTLs_nominal_agg_best {
    ...
    """
    zcat trans.nominal.chunk_${sample_ID}*.best.txt.gz > trans.nominal.chunk_${sample_ID}_best.txt
    gzip -c trans.nominal.chunk_${sample_ID}_best.txt > trans.nominal_${sample_ID}_best.txt.gz
...
process trans_eQTLs_permutational_agg_hits {
    ...
    zcat trans_${sample_ID}_perm.chunk_\${i}_*.hits.txt.gz > trans_perm_${sample_ID}_hits/trans_${sample_ID}_perm.chunk_\${i}_hits.txt
...
process trans_eQTLs_permutational_agg_bins {
    ...
    zcat trans_${sample_ID}_perm.chunk_\${i}_*.bins.txt.gz > trans_perm_${sample_ID}_bins/trans_${sample_ID}_perm.chunk_\${i}_bins.txt
...
process trans_eQTLs_permutational_agg_best {
    ...
    zcat trans_${sample_ID}_perm.chunk_\${i}_*.best.txt.gz > trans_perm_${sample_ID}_best/trans_${sample_ID}_perm.chunk_\${i}_best.txt
...
workflow {
    ...
    cis_eQTLs_conditional_for_correct(cis_eQTLs_conditional.out.join(merge_check.out[0], by: 0), params.script_snps_extraction)
    cis_residualise(metadata_residualize.out[0].join(cis_eQTLs_conditional_for_correct.out.join(cis_eQTLs_conditional.out, by: 0), by: 0), params.script_cisresid, params.No_peer_factors)
